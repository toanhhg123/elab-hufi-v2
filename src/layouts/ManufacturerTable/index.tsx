...
import {
  ...
  postManufacturer,
  updateManufacturer,
} from "../../services/manufacturerServices";
...
const ManufacturersTable: FC = () => {
  const manufacturersData = useAppSelector(
    (state: RootState) => state.manufacturer.listOfManufacturers
  );
  ...
  const handleSubmitEditModal = async () => {
    ...
    if (isUpdatedSuccess) {
      dispatch(
        setSnackbarMessage("Cập nhật thông tin nhà sản xuất thành công")
      );
      let updatedIdx = manufacturersData.findIndex(
        (x) => x.ManufacturerId === updatedRow.ManufacturerId
      );
      let newListOfManufacturers = [
        ...manufacturersData.slice(0, updatedIdx),
        updatedRow,
        ...manufacturersData.slice(updatedIdx + 1),
      ];
  ...
  const handleSubmitDeleteModal = async () => {
    ...
    let deletedIdx = manufacturersData.findIndex(
      (x) => x.ManufacturerId === deletedRow.ManufacturerId
    );
    let newListOfManufacturers = [
      ...manufacturersData.slice(0, deletedIdx),
      ...manufacturersData.slice(deletedIdx + 1),
    ];
    dispatch(setListOfManufacturers(newListOfManufacturers));
  ...
  const handleSubmitCreateModal = async () => {
    ...
    if (createdManufacturer) {
      const newListOfManufacturers: IManufacturerType[] =
        await getManufacturers();
      if (newListOfManufacturers) {
        dispatch(
          setSnackbarMessage("Tạo thông tin nhà sản xuất mới thành công")
        );
        dispatch(setListOfManufacturers(newListOfManufacturers));
      }
    }
    ...
    <>
      ...
      <Dialog open={isEditModal}>
        <DialogTitle textAlign="center">
          <b>Sửa thông tin nhà sản xuất</b>
        </DialogTitle>
        <DialogContent>
          <form
      ...
      <Dialog open={isDeleteModal}>
        <DialogTitle textAlign="center">
          <b>Xoá thông tin nhà sản xuất</b>
        </DialogTitle>
        <DialogContent>
          <div>
            Bạn có chắc muốn xoá thông tin nhà sản xuất{" "}
            {`${deletedRow.ManufacturerId}`} không?
          </div>
        </DialogContent>
        <DialogActions sx={{ p: "1.25rem" }}>
      ...
      <Dialog open={isCreateModal}>
        <DialogTitle textAlign="center">
          <b>Tạo thông tin nhà sản xuất</b>
        </DialogTitle>
        <DialogContent>
          <form
            onSubmit={(e) => e.preventDefault()}
            ...
            >
              {columns.map((column) => (
                <TextField
                  ...
                  name={column.accessorKey}
                  defaultValue={
                    column.accessorKey && createdRow[column.accessorKey]
                  }
                  onChange={(e) =>
                    setCreatedRow({
                      ...createdRow,
