...
const DeviceOfDepartmentTable = () => {
  ...
  const [isOpenDeviceUsageHours, setIsOpenDeviceUsageHours] =
    useState<boolean>(false);
  const [isOpenDeviceLiquidate, setIsOpenDeviceLiquidate] =
    useState<boolean>(false);
  ...
  const handleSubmitDelete = async (DeviceId: String) => {
    try {
      ...
      if (data) {
        ...
        const newData = deviceData[deviceType]?.filter(
          (device: any) => DeviceId !== device?.DeviceId
        );
        setDeviceDataValues({
          ...deviceData,
          [deviceType]: newData,
  ...
  return (
    <>
      ...
      {isOpenDeviceLiquidate && (
        <DialogLiquidate
          isOpen={isOpenDeviceLiquidate}
          onClose={() => setIsOpenDeviceLiquidate(false)}
        />
      )}
...
const RowDevice = ({ device, isOpen, handleClose }: RowDeviceProps) => {
  ...
  const [maintenanceDevice, setMaintenanceDevice] =
    useState<IRepairDevice | null>(null);
  const [isOpenMaintenance, setIsOpenMaintenance] = useState<boolean>(false);
  const [deviceHitories, setDeviceHistories] = useState<
    IDeviceHistory | IInstrumentHistory | null
  >(null);
  ...
  const getMaintenanceDevice = async () => {
    setLoading(true);
    try {
      let maintenanceDevice: IRepairDevice = await getMaintenanceDeviceById(
        serialNumber
      );
      ...
      if (maintenanceDevice) {
        ...
        let index = deviceDetails.findIndex(
          (x) => x?.DeviceInfoId === serialNumber
        );
        // if (index !== -1) {
  ...
  const getHistoryDevice = async () => {
    setLoading(true);
    try {
      let deviceHitories: IDeviceHistory = await getDeviceHitories(
        serialNumber
      );
  ...
  const getHistoryInstrument = async () => {
    ...
    setLoading(true);
    try {
      let deviceHitories: IInstrumentHistory = await getInstrumentHitories(
        serialNumber
      );
      setDeviceHistories(deviceHitories);
    } catch (error) {
