...
const DepartmentTable: FC = () => {
  ...
  const handleSubmitEditModal = async () => {
    ...
    if (isUpdatedSuccess) {
      ...
      let updatedIdx = departmentData.findIndex(
        (x) => x.DepartmentId === updatedRow.DepartmentId
      );
  ...
  const handleSubmitDeleteModal = async () => {
    ...
    let deletedIdx = departmentData.findIndex(
      (x) => x.DepartmentId === deletedRow.DepartmentId
    );
    let newListOfDepartments = [
      ...departmentData.slice(0, deletedIdx),
      ...departmentData.slice(deletedIdx + 1),
    ];
    dispatch(setListOfDepartments(newListOfDepartments));
  ...
  return (
    <>
      ...
      <Dialog open={isDeleteModal}>
        ...
        <DialogContent>
          <div>
            Bạn có chắc muốn xoá thông tin phòng ban{" "}
            {`${deletedRow.DepartmentName}`} không?
          </div>
        </DialogContent>
        <DialogActions sx={{ p: "1.25rem" }}>
