...
const ResearchTeamTable: FC = () => {
  const researchTeamsData = useAppSelector(
    (state: RootState) => state.researchTeam.listOfResearchTeams
  );
  const { currentResearchTeam, currentMemberTeam } = useAppSelector(
  ...
  const [isDeleteMemberTeamModal, setIsDeleteMemberTeamModal] =
    useState<boolean>(false);
  const [isEditMemberTeamModal, setIsEditMemberTeamModal] =
    useState<boolean>(false);
  const [isCreateMemberTeamModal, setIsCreateMemberTeamModal] =
    useState<boolean>(false);
  ...
  const handleSubmitEditModal = async () => {
    ...
    if (isUpdatedSuccess) {
      dispatch(
        setSnackbarMessage("Cập nhật thông tin nhóm nghiên cứu thành công")
      );
      let updatedIdx = researchTeamsData.findIndex(
        (x) => x.TeamId === currentResearchTeam.TeamId
      );
  ...
  const handleSubmitCreateModal = async () => {
    ...
    const createdResearchTeam = await postResearchTeam(
      currentResearchTeamClone
    );
    if (createdResearchTeam.TeamId) {
      const newListOfResearchTeams: IResearchTeamType[] =
        await getResearchTeams();
      if (newListOfResearchTeams) {
        dispatch(
          setSnackbarMessage("Tạo thông tin nhóm nghiên cứu mới thành công")
        );
        dispatch(setListOfResearchTeams(newListOfResearchTeams));
      }
    }
  ...
  const handleSubmitDeleteMemberTeamModal = () => {
    let deletedIdx = currentResearchTeam.listMember.findIndex(
      (item: IResearcherType) =>
        item.ResearcherId === currentMemberTeam.ResearcherId
    );
    if (deletedIdx > -1) {
      dispatch(
        setCurrentResearchTeam({
        ...
        renderRowActions={({ row, table }) => (
          <>
            ...
            <Tooltip
              arrow
              placement="right"
              title="Xoá thông tin nhóm nghiên cứu"
            >
              <IconButton
                color="error"
