...
const EditMemberTeamModal: FC<{
  ...
  const { currentMemberTeam } = useAppSelector(
              ...
              if (column.accessorKey === "formatedBirthdate") {
                return (
                  <LocalizationProvider dateAdapter={AdapterMoment}>
                    <DatePicker
                      ...
                      label="Ngày sinh"
                      value={
                        new Date(Number(currentMemberTeam.Birthdate) * 1000)
                      }
                      onChange={(val: any) =>
                        dispatch(
                ...
                return (
                  <TextField
                    ...
                    defaultValue={
                      column.accessorKey &&
                      currentMemberTeam[column.accessorKey]
                    }
                  />
                );
                ...
                return (
                  <FormControl sx={{ m: 0, minWidth: 120 }}>
                    ...
                    <Select
                      ...
                      id="edit-select-title-required"
                      value={
                        currentMemberTeam?.Title &&
                        ResearchTeamTitles[currentMemberTeam.Title].toString()
                      }
                      label="Chức vụ"
                      onChange={(e: SelectChangeEvent) =>
                        dispatch(
                          setCurrentMemberTeam({
                            ...currentMemberTeam,
                            Title: ResearchTeamTitles[
                              Number(e.target.value)
                            ] as keyof typeof ResearchTeamTitles,
                          })
                        )
                      }
                    >
                      {Object.values(ResearchTeamTitles)
                        .slice(
                          0,
                          (Object.values(ResearchTeamTitles).length + 1) / 2
                        )
                        .map((x, idx) => (
                          <MenuItem key={"UpdateTitle" + idx} value={idx}>
                            {x}
                          </MenuItem>
                        ))}
                    </Select>
