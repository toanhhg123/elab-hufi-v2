...
const ResearchersTable: FC = () => {
    ...
    const handleSubmitEditModal = async () => {
        ...
        if (isUpdatedSuccess) {
            dispatch(setSnackbarMessage("Cập nhật thông tin nhà nghiên cứu thành công"));
            let updatedIdx = researchersData.findIndex(x => x.ResearcherId === updatedRow.ResearcherId);
            let newListOfResearchers = [...researchersData.slice(0, updatedIdx), updatedRow, ...researchersData.slice(updatedIdx + 1,)]
    ...
    const handleSubmitDeleteModal = async () => {
        ...
        let deletedIdx = researchersData.findIndex(x => x.ResearcherId === deletedRow.ResearcherId);
        let newListOfResearchers = [...researchersData.slice(0, deletedIdx), ...researchersData.slice(deletedIdx + 1,)]
        dispatch(setListOfResearchers(newListOfResearchers));
    ...
    const handleSubmitCreateModal = async () => {
        ...
        if (createdResearcher) {
            ...
            if (newListOfResearchers) {
                dispatch(setSnackbarMessage("Tạo thông tin nhà nghiên cứu mới thành công"));
                dispatch(setListOfResearchers(newListOfResearchers));
            }
        }
        onCloseCreateModal();
        ...
        <>
            <MaterialReactTable
                ...
                renderRowActions={({ row, table }) => (
                    <>
                        <Tooltip arrow placement="left" title="Sửa thông tin nhà nghiên cứu">
                            <IconButton onClick={() => handleOpenEditModal(row)}>
                                <Edit />
                            </IconButton>
                        </Tooltip>
                        <Tooltip arrow placement="right" title="Xoá thông tin nhà nghiên cứu">
            ...
            <Dialog open={isEditModal}>
                <DialogTitle textAlign="center"><b>Sửa thông tin nhà nghiên cứu</b></DialogTitle>
                <DialogContent>
            ...
            <Dialog open={isDeleteModal}>
                <DialogTitle textAlign="center"><b>Xoá thông tin nhà nghiên cứu</b></DialogTitle>
                <DialogContent>
                    <div>Bạn có chắc muốn xoá thông tin nhà nghiên cứu {`${deletedRow.ResearcherId}`} không?</div>
                </DialogContent>
                <DialogActions sx={{ p: '1.25rem' }}>
            ...
            <Dialog open={isCreateModal}>
                <DialogTitle textAlign="center"><b>Tạo thông tin nhà nghiên cứu</b></DialogTitle>
                <DialogContent>
                    <form onSubmit={(e) => e.preventDefault()} style={{ "marginTop": "10px" }}>
                        <Stack
                            sx={{
