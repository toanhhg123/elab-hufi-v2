...
const ExportChemRegTable: FC<{ exportChemReg: IExportChemRegType[]; columns: ColumnType[]; }> = ({ exportChemReg, columns }) => {
  const [tableData, setTableData] = useState<IExportChemRegType[]>(exportChemReg);
  ...
  useEffect(() => {
    setTableData(prev => {
      let data = [...prev];
      data?.sort((a: IExportChemRegType, b: IExportChemRegType) => {
        let i =
          order === 'desc'
  ...
  useEffect(() => {
    const exportChemRegItems: IExportChemRegType[] = exportChemReg || [];
    const data = exportChemRegItems?.map((x: IExportChemRegType) => {
      let string: String = '';
  ...
  useEffect(() => {
    ...
    if (keyword === '') {
      setTableData(exportChemRegItems);
    } else {
      const data = exportChemRegItems?.filter((x: any) => listId.indexOf(x?.RegisterGeneralId) !== -1);
      setTableData(data);
    }
  }, [keyword, dataSearch]);
  ...
  return (
    <>
      <Box component="div" alignItems="center" justifyContent="space-between" display="flex" mb={2}>
        <Typography fontWeight="bold">Thông tin xuất hóa chất cho nghiên cứu</Typography>
      ...
      <TableContainer component={Paper} sx={{ maxHeight: '400px', marginBottom: '24px', overflow: 'overlay' }}>
        <Table sx={{ minWidth: 650 }} stickyHeader size="small">
          ...
          <TableBody>
            {tableData.length > 0 ? tableData?.map((chemDeptItem: IExportChemRegType, index: number) => (
