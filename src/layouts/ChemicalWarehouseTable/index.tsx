...
import DialogLiquidate from './Dialog/DialogLiquidate';
...
const ChemicalWarehouseTable: FC<{ role: number }> = ({ role }) => {
	const chemicalWarehouseData: IChemicalWarehouseType[] = useAppSelector(
		(state: RootState) => state.chemicalWarehouse.listOfChemicalWarehouse,
	);

	const [tableData, setTableData] = useState<IChemicalWarehouseType[]>([]);
	const [isOpenLiquidate, setIsOpenLiquidate] = useState<boolean>(false);
	const [validationErrors, setValidationErrors] = useState<{
		[cellId: string]: string;
	}>({});

	useEffect(() => {
		let formatedChemicalWarehouseData =
			role === 1
				? chemicalWarehouseData.length > 0
					? chemicalWarehouseData.map(item =>
							Object.assign(
								{},
								{
									...item,
	...
	return (
		<>
			{isOpenLiquidate && <DialogLiquidate isOpen={isOpenLiquidate} onClose={() => setIsOpenLiquidate(false)} />}
			<MaterialReactTable
				...
				initialState={{
					density: 'compact',
					columnOrder: ['mrt-row-expand', 'mrt-row-numbers', ...columns.map(item => item.accessorKey || '')],
				}}
				...
				renderToolbarInternalActions={({ table }) => (
					<>
						<Button variant="contained" onClick={() => setIsOpenLiquidate(true)}>
							Thanh lý hóa chất
						</Button>
